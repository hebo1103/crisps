...
public class RedisMessageListener extends MessageListenerAdapter {
    ...
    public void onMessage(Message message, byte[] pattern) {
        ...
        RedisPubSubMessage redisPubSubMessage = (RedisPubSubMessage) cacheManager.getRedisTemplate().getValueSerializer().deserialize(message.getBody());
        ...
        for (Cache cache : caches) {
            // 判断缓存是否是多级缓存
            if (cache != null && cache instanceof CacheMe) {
                switch (redisPubSubMessage.getMessageType()) {
                    case EVICT:
                        // 获取一级缓存，并删除一级缓存数据
//                        ((CacheMe) cache).getFirstCache().evict(redisPubSubMessage.getKey());
                        log.info("删除一级缓存{}数据,key={}", redisPubSubMessage.getCacheName(), redisPubSubMessage.getKey());
                        break;

                    case CLEAR:
                        // 获取一级缓存，并删除一级缓存数据
//                        ((CacheMe) cache).getFirstCache().clear();
                        break;
                    case CLEAR_ALL:
                        cache.outherClear();
                        break;
