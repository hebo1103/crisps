...
import net.crisps.cloud.framework.cache.core.cache.AbstractValueAdaptingCache;
import net.crisps.cloud.framework.cache.core.setting.SecondaryCacheSetting;
import net.crisps.cloud.framework.cache.core.support.AwaitThreadContainer;
import net.crisps.cloud.framework.cache.core.support.Lock;
import net.crisps.cloud.framework.cache.core.support.NullValue;
import net.crisps.cloud.framework.cache.core.support.ThreadTaskUtils;
...
//            refreshCache(redisCacheKey, valueLoader, result);
        ...
        for (int i = 0; i < RETRY_COUNT; i++) {
            try {
                ...
                if (redisLock.lock()) {
                    ...
                    logger.debug("redis缓存 key= {} 从持久层获取数据完毕，唤醒所有等待线程", redisCacheKey.getKey());
                    // 唤醒线程
    ...
    private <T> T loaderAndPutValue(RedisCacheKey key, Callable<T> valueLoader, boolean isLoad) {
        ...
        try {
            ...
            logger.debug("redis缓存 key={} 执行被缓存的方法, 耗时：{}。数据:{}", key.getKey(), System.currentTimeMillis() - start, JSON.toJSONString(result));
            if (isLoad && isStats()) {
