...
import net.crisps.cloud.framework.cache.core.listener.RedisMessageListener;
...
public abstract class AbstractCacheManager implements CacheManager, InitializingBean, DisposableBean, BeanNameAware, SmartLifecycle {
    ...
    public Cache getCache(String name, CacheSetting cacheSetting) {
        ...
        if (!CollectionUtils.isEmpty(cacheMap)) {
            ...
            Cache cache = cacheMap.get(cacheSetting.getInternalKey());
            if (cache != null) {
                return cache;
            }
        }
        ...
        synchronized (this.cacheContainer) {
            ...
            // 新建一个Cache对象
            Cache cache = getMissingCache(name, cacheSetting);
            if (cache != null) {
                // 装饰Cache对象
